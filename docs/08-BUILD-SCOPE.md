...
## Wave 0-8 실행 결과 (Wave 8 Task 8.4 sync)

Day-1 빌드 실제 진행 기록. 각 Wave 커밋은 git log에서 확인 가능.

| Wave | 실제 내용 | 테스트 | 주요 drift / carry-forward |
|---|---|---|---|
| 0 | 스캐폴딩 (10 crate + Python 패키지 + 디렉토리) | - | - |
| 1 | Layer 0-1 (myth-common + myth-db schema v1) | +N | - |
| 2 | Layer 2 (myth-gavel + myth-identity + myth-embed) | +N | Gavel fields Arc → Box 변경 (승인) |
| 3 | Layer 3 (myth-hooks 6 bins, Tier 0 classifier) | +N | Tier 0 concurrent coverage gap (→ Wave 7에서 해소) |
| 4 | Layer 4 (myth-runtime + myth-orchestrator + myth-ui) | +N | - |
| 5 | Layer 5 (myth-cli 13 subcommands + myth bin) | +N | docs/10 drift 7건 (→ Wave 5.5 sync) |
| 5.5 | docs/04-CRATES/10-myth-cli.md drift sync | 0 | closed |
| 6 | Python layer (assessor + observer + tests) | +36 Python | drift 7건 (6 문서만 sync, 1 closed) |
| 7 | 규칙 + fixtures + harness + fs2→fs4 + Tier 0 concurrent | +7 (268→275) | carry-forward 2 closed + 1 new (migration race) + drift sub-1a/1b/1c |
| 8 | lesson split/merge + Python install + Tier 3 wiring + docs sync + LICENSE + e2e + tag | Wave 8 누적 | carry-forward 3,4 closed + schema v1 유지 (Option B) |

**Wave 8 종료 시점 누적 테스트** (목표): Rust ~278+, Python 36.

**Wave 8 종료 시점 carry-forward 잔존**:
- ~~#1 fs2 → fs4~~ (Wave 7 closed)
- ~~#2 Tier 0 concurrent coverage~~ (Wave 7 closed)
- ~~#3 assessor ↔ myth-hooks Tier 3~~ (Wave 8 Task 8.3 closed, 구조만)
- ~~#4 install.rs Python auto~~ (Wave 8 Task 8.2 closed)
- **#5 migration cold-start race → Milestone C 연기**
- **#6 lesson_relations 인덱스 최적화 → Milestone C 또는 Wave 1 v2 migration**

**회고 원칙 재발 카운트**: Wave 1-7 연속 0회 (Wave 8 기준점 수립 중).
